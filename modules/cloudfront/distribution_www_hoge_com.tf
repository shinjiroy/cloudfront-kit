resource "aws_cloudfront_distribution" "www_hoge_com" {
  aliases = [var.www_hoge_com_domain_name]
  comment = var.www_hoge_com_domain_name
  ...
  origin {
    ...
    domain_name = var.hoge_alb_domain_name
  ...
  origin {
    connection_attempts = 3
    connection_timeout = 10
    domain_name = data.aws_s3_bucket.hoge_bucket.bucket_regional_domain_name
    origin_id = "hoge_bucket"
    origin_path = null
    origin_access_control_id = aws_cloudfront_origin_access_control.hoge_bucket.id
  }
